...
struct WorldTimeAPIResult {
	...
	*/
	char timezone[TIME_ZONE_INFO_TZ_NAME_SIZE];

	/**
	* @brief Abbreviation name of time zone.
	*/
	char abbreviation[TIME_ZONE_INFO_TZ_ABR_NAME_SIZE];
	...
	*/
	inline bool hasError() const {
		return httpCode != WorldTimeAPI_HttpCode::HTTP_CODE_OK;
	}
...
protected:
	...
	*/
	...
	static bool jsonItemTZ(JSONItemType type, const char* key, int keyLength, const SimpleJSONParser::Number& parsedVal, int depth, int index, void* owner_ptr);
	...
	static bool jsonItemERR(JSONItemType type, const char* key, int keyLength, const SimpleJSONParser::Number& parsedVal, int depth, int index, void* owner_ptr);
